# Banque de la République d'Haïti Exchange Rates API — brh-exchange-rate

[![npm version](https://img.shields.io/npm/v/brh-exchange-rate.svg)](https://www.npmjs.com/package/brh-exchange-rate)
[![license](https://img.shields.io/npm/l/brh-exchange-rate.svg)](https://github.com/AllRates-Today/brh-exchange-rate/blob/main/LICENSE)
[![zero dependencies](https://img.shields.io/badge/dependencies-0-brightgreen.svg)](https://www.npmjs.com/package/brh-exchange-rate)
[![TypeScript](https://img.shields.io/badge/TypeScript-types%20included-3178C6.svg)](https://www.typescriptlang.org/)

**Official Banque de la République d'Haïti (Haiti) daily exchange rates for Node.js and TypeScript. The published central bank rates behind tax filings, customs valuations, audits, and compliant invoicing — not market estimates, but the numbers Banque de la République d'Haïti itself prints, every business day.**

## 🚀 Why this client?

- 🏛️ **Official published rates** — Banque de la République d'Haïti's own table, with the publisher's own `rate_date` on every response
...
> **Official rate, not mid-market:** every value here is a number Banque de la République d'Haïti itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Banque de la République d'Haïti table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/brh?source=USD&target=HTG"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/brh').then((x) => x.json());
...
```bash
yarn add brh-exchange-rate
```

```bash
pnpm add brh-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/brh-exchange-rate`](https://www.npmjs.com/package/@allratestoday/brh-exchange-rate) — same code, same versions.
...
console.log(pair.rate, pair.rate_date); // the official Banque de la République d'Haïti rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'brh-exchange-rate';
...
```js
import { getRatesForDate } from 'brh-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'USD', target: 'HTG' });
```
...
Banque de la République d'Haïti currently publishes rates covering **1 currency** against the HTG (as of the latest table):
...
- Publisher's own page: [Taux du jour](https://www.brh.ht/taux-du-jour/) · [www.brh.ht](https://www.brh.ht)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Banque de la République d'Haïti rates page](https://allratestoday.com/central-bank-rates-api/brh/)
...
- Banque de la République d'Haïti quotes **HTG per 1 unit of foreign currency** (e.g. `base: "USD", quote: "HTG"` means HTG per one US dollar).
...
Loading the official Banque de la République d'Haïti rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/brh/latest?format=xml&api_key=art_live_..."
```
...
If Banque de la République d'Haïti does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via HTG from two published rates |
...
| `404` | Pair or date range not covered by Banque de la République d'Haïti |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'brh-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('brh-exchange-rate');

getRate('USD', 'HTG', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- [Banque de la République d'Haïti rates page](https://allratestoday.com/central-bank-rates-api/brh/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/brh-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/brh-exchange-rate)
